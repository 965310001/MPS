 ...
 *
 * -->解决BUG
 * * FragmentUserVisibleController还专门解决了在Fragment或ViewPager嵌套ViewPager时子Fragment的mUserVisibleHint
 *      属性与父Fragment的mUserVisibleHint属性不同步的问题
 * * 例如外面的Fragment的mUserVisibleHint属性变化时，其包含的ViewPager中的Fragment的mUserVisibleHint属性并不会随着改变，
 *      这是ViewPager的BUG
 *
 ...
 *      分别调用FragmentUserVisibleController的activityCreated()、resume()、pause()、setUserVisibleHint(boolean)、onHiddenChanged(boolean)方法
 * 3. 实现FragmentUserVisibleController.UserVisibleCallback接口并实现以下方法
 *      void setWaitingShowToUser(boolean)：
 ...
 *      void callSuperSetUserVisibleHint(boolean)：
 *          调用父Fragment的setUserVisibleHint(boolean)方法即可
 *      void onVisibleToUserChanged(boolean, boolean)：
...
public class FragmentUserVisibleController {
    ...
    public void onHiddenChanged(boolean onHiddenChange) {
        setUserVisibleHint(!onHiddenChange);
    }
