...
public class StackLabel<T> extends RelativeLayout {
    ...
    private StackLabelAdapter<T> adapter;
    ...
    public void setData(List<T> data) {
        if (adapter != null) {
            adapter.setLabels(data);
        } else {
            throw new NullPointerException("必须先调用setAdapter()方法，才能调用setData()方法！");
        }
    }

    public StackLabel<T> setAdapter(StackLabelAdapter.CovertData<T> covertData) {
        this.adapter = new StackLabelAdapter<>(covertData);
        this.adapter.setStackLabel(this);
        this.adapter.notifyDataSetChanged();
        return this;
    }

    public StackLabel<T> setAdapter(StackLabelAdapter<T> adapter) {
    ...
    public void notifyDataSetChanged() {
        if (adapter == null) {
            throw new NullPointerException("必须先调用setAdapter()方法，notifyDataSetChanged()方法！");
        }
        removeAllViews();
        items = new ArrayList<>();
